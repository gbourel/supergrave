...
# Une seule dent
def dent(cx, cy, angle, Dpied, Dp, Dtete, pasAngulaire):
  x = int(cx + math.cos(angle) * (Dpied / 2) * pr)
  y = int(cy + math.sin(angle) * (Dpied / 2) * pr)
  print("MOVE", x, y)
  x = int(cx + math.cos(angle) * (Dp / 2) * pr)
  y = int(cy + math.sin(angle) * (Dp / 2) * pr)
  print("MOVE", x, y)
  angle += pasAngulaire / 8
...
def roueDentee(cx, cy, dents, module):
  ...
  for i in range(dents):
    dent(cx, cy, alpha, Dpied, Dp, Dtete, pa)
    alpha += pa
  print("MOVE", int(cx + (Dpied / 2) * pr), cy)
  print("LASER OFF")
